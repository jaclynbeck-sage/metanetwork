...
tigress <- function(y, x) {
  ...
  for (i in 1:floor(R / 2)) {
    ...
    result1 <- lars::lars(
      x = xr1,
      y = y[indexVec[1:floor(n / 2)]],
      type = 'lar',
      max.steps = L,
      use.Gram = FALSE
    )
    w1 <- rev(order(result1$entry, decreasing = T)[1:L])
    indexMat[, w1][lower.tri(indexMat[, w1], diag = T)] <- indexMat[, w1][lower.tri(indexMat[, w1], diag =
                                                                                      T)] + 1

    result2 <- lars::lars(
      x = xr2,
      y = y[indexVec[(floor(n / 2) + 1):n]],
      type = 'lar',
      max.steps = L,
      use.Gram = FALSE
    )
    w2 <- rev(order(result2$entry, decreasing = T)[1:L])
    indexMat[, w2][lower.tri(indexMat[, w2], diag = T)] <- indexMat[, w2][lower.tri(indexMat[, w2], diag =
                                                                                      T)] + 1
