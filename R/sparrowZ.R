...
#' @export
sparrowZ <- function(y, x, ordering_mat = NULL, eps = 1e-6, exclude = NULL,
                     add.intercept = TRUE, maxit = 1e4, n_orderings = 10,
                     family = "normal", scaling = TRUE, return_kl = TRUE,
                     estimation_type = "BMA", bma_approximation = TRUE,
                     screen = 1.0, post = 0.95, already_screened = 1.0,
                     kl = 0.99, l0_path = NULL, cleanSolution = FALSE) {
  result <- vbsr::vbsr(
    y = y,
    X = x,
    ordering_mat = ordering_mat,
    eps = eps,
    exclude = exclude,
    ...
    bma_approximation = bma_approximation,
    screen = screen,
    post = post,
